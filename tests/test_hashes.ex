...
/* tested with openssl */
constant gost94_cryptopro_tests = {
...
ifdef USE_KECCAK then
constant keccak_224_tests = {
...
end ifdef /* USE_KECCAK */

/* verified by eBASH SUPERCOP implementation */
constant edonr256_tests = {
...
constant btih_with_filename_tests = {
...
constant btih_without_a_filename_tests = {
...
function rhash_string( integer hash_id, sequence message )
	
	object bytes = rhash_msg( hash_id, message )
...
procedure verbose( integer fn, sequence msg, object data = {} )
	ifdef VERBOSE then printf( fn, msg, data ) end ifdef
end procedure
...
procedure main()
	...
	for i = 1 to length( short_test_vectors ) do
		
		integer hash_id = short_test_vectors[i][1]
		sequence test_vectors = short_test_vectors[i][2]
		
		printf( 1, "%s\n", {rhash_get_name(hash_id)} )
		verbose( 1, "\n" )
		
		for j = 1 to length( test_vectors ) by 2 do
			
			sequence message = test_vectors[j]
			sequence digest = test_vectors[j+1]
			
			verbose( 1, "message: \"%s\"\n", {message} )
			verbose( 1, "digest: %s\n", {digest} )
			
			object result = rhash_string( hash_id, message )
			
			if atom( result ) then
				printf( 2, "rhash_msg() failed\n" )
				return
			end if
			
			verbose( 1, "result: %s\n", {result} )
			
			if not equal( result, digest ) then
				printf( 2, "invalid result\n" )
				return
			end if
